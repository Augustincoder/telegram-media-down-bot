...
async def save_profile_handler(message: Message, session: AsyncSession):
    ...
    if not username:
        await message.answer(
            "❌ Noto'g'ri format! Iltimos, /save @username yoki /save link ko'rinishida yuboring."
        )
        return
    ...
    try:
        ...
        new_profile = SavedProfile(
            user_id=user_id, ig_username=user_info.username, ig_user_id=str(user_info.pk)
        )
        session.add(new_profile)
        await session.commit()

        await wait_msg.edit_text(
            f"✅ <b>@{user_info.username}</b> muvaffaqiyatli saqlandi! Ularni ko'rish uchun /saved ni bosing."
        )
    except Exception:
        await wait_msg.edit_text("❌ Profilni topib bo'lmadi. Username to'g'riligini tekshiring.")
            ...
            keyboard.append(
                [
                    InlineKeyboardButton(
                        text=f"⚠️ @{p.ig_username}", callback_data=f"get_story_{p.id}"
                    )
                ]
            )
