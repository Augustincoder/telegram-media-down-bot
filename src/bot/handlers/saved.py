...
async def process_story_request(callback: CallbackQuery, session: AsyncSession):
    ...
    await callback.message.answer(f"📥 <b>@{profile.ig_username} {emoji}</b> hikoyalari tortilmoqda...")
    ...
    try:
        if profile.platform == "instagram":
            ...
            for story in stories:
                ...
                cache_res = await session.execute(select(StoryCache).where(StoryCache.story_id == story_pk, StoryCache.platform == "instagram"))
                cached = cache_res.scalar_one_or_none()
                if cached and config.storage_channel_id:
                    try:
                        await bot.copy_message(chat_id=callback.from_user.id, from_chat_id=config.storage_channel_id, message_id=cached.telegram_msg_id)
                        continue
                    except Exception:
                        pass
            ...
            for story in stories:
                ...
                cache_res = await session.execute(select(StoryCache).where(StoryCache.story_id == story_id, StoryCache.platform == "telegram"))
                cached = cache_res.scalar_one_or_none()
                if cached and config.storage_channel_id:
                    try:
                        await bot.copy_message(chat_id=callback.from_user.id, from_chat_id=config.storage_channel_id, message_id=cached.telegram_msg_id)
                ...
                try:
                    downloaded = await userbot_service.download_story(profile.ig_username, story.id, file_path)
                    if downloaded:
