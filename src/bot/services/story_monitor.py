    ...
    while True:
        try:
            ...
            async with AsyncSessionLocal() as session:
                ...
                result = await session.execute(select(SavedProfile.ig_user_id, SavedProfile.ig_username).distinct())
                profiles = result.all()
                ...
                for ig_user_id, ig_username in profiles:
                    ...
                    try:
                        stories = await loop.run_in_executor(None, ig_service.client.user_stories, ig_user_id)
